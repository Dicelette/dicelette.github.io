...
const StatisticsRow = ({
  ...
  const minErrClass = useMemo(() => minimalErrorClass(stat), [stat.min, stat.max, stat.combinaison]);
  const maxErrClass = useMemo(() => maximalErrorClass(stat), [stat.min, stat.max, stat.combinaison]);

  return (
    <Draggable key={stat.id || statIndex} draggableId={String(stat.id || statIndex)} index={statIndex}>
      {(provided) => (
        <tr
        ...
        >
          ...
          <td className="p-2 min-[0px]:max-2xl:w-full ">
            <StandaloneToggleButton
              selected={!!excluded}
              onChange={() => setFieldValue(`statistics[${statIndex}].excluded`, !excluded)}
              size={1280}
              opt={"excludedStat"}
            />
          </td>
