...
const CustomCriticalRow = ({
  ...
  return (
    <Draggable key={custom.id || index} draggableId={String(custom.id || index)} index={index}>
      {(provided) => (
        <tr
        ...
        >
          ...
          <td className="p-2 min-[0px]:max-xl:w-full">
            <Autocomplete
              ...
              className={"autocomplete 2xl:w-[200px] !mb-0 min-[0px]:max-2xl:w-full"}
              style={{ fontFamily: "Fira Code VF" }}
              options={[">", ">=", "<", "<=", "==", "!="]}
              value={selection || ""}
              onChange={(e, newValue) => setFieldValue(`customCritical[${index}].selection`, newValue || "")}
              renderInput={(params) => (
                <TextField
                  size={"medium"}
                  {...params}
            ...
            {errorTooltip({ index, idName: "selection", duplicateIndices, customCritical: custom })}
          </td>
            <td className="p-2 min-[0px]:max-xl:w-full">
              <Tablefield
              ...
              {errorTooltip({ index, idName: "name", duplicateIndices, customCritical: custom })}
            </td>
            <td className="p-2 min-[0px]:max-xl:w-full h-200px">
              ...
              {errorTooltip({ index, idName: "formula", duplicateIndices, customCritical: custom })}
            </td>
            <td className="p-2 min-[0px]:max-xl:w-full h-200px">
              <StandaloneToggleButton
                selected={onNaturalDice}
                onChange={() => setFieldValue(`customCritical[${index}].onNaturalDice`, !onNaturalDice)}
                size={1280}
                opt={"naturalDice"}
              />
            </td>
            <td className="p-2 min-[0px]:max-xl:w-full h-200px">
              <StandaloneToggleButton
                ...
                onChange={() => setFieldValue(`customCritical[${index}].affectSkill`, !affectSkill)}
                size={1280}
                opt={"affectSkill"}
              />
            </td>
